#!/usr/bin/env python3
"""Prove exact rc.4 -> candidate -> rc.4 rollback in a disposable home."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run both exact portable installers in one disposable home, "
            "prove rollback bytes/modes, uninstall, and emit non-secret JSON."
        )
    )
    parser.add_argument("--restore-bundle", type=Path, required=True)
...
def main() -> None:
    ...
    try:
        report = prove_cross_version_rollback(
            restore_bundle=args.restore_bundle,
