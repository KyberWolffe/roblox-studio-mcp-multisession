# Roblox Studio MCP v2 — install, update, and recovery

This package installs Roblox Studio MCP v2 beside the existing v1 integration.
It never replaces the v1 executable, plugin, or Codex table.
...
For a public repository, use the versioned bootstrap from an exact tag and the
two hashes published with that GitHub Release. The repository README contains
...
version. If a different v2 version is already installed, use the installed
manager's exact-tag `update` command below. No command-line flag can bypass
the live transaction nonce and exact on-disk from/to version fence.
...
Private-repository authentication stays outside v2. Use an already
...
installer arguments, or v2 configuration.
...
[mcp_servers.Roblox_Studio_v2] in $HOME/.codex/config.toml
```

It refuses to adopt an unowned v2 Codex table, backs up the config
byte-for-byte before editing it, and leaves
`[mcp_servers.Roblox_Studio]` outside v2 ownership. Machine credentials are
generated during installation, stored only in `config/secrets.json` with mode
`0600`, and never shipped.

After installation:

1. restart Codex to load/refresh `Roblox_Studio_v2`;
...
In normal tasks, users name the desired place/project. Codex discovers the v2
sessions and supplies the matching internal `studio_id`; users do not copy IDs,
select a global active Studio, manage locks, or start a broker manually. Codex
asks only when duplicate names or unsaved documents are genuinely ambiguous.
Every operational call remains explicitly session-targeted internally.
...
"$HOME/Library/Application Support/RobloxStudioMCPv2/bin/roblox-studio-mcp-v2-manage" repair
```

Repair is idempotent. It reconstructs damaged v2-owned release, launcher,
catalog, or plugin files from the installed package. It will not overwrite a
modified Codex v2 table without `--replace-owned-config`, and it never edits
v1. Missing/damaged credentials fail closed; deliberate
...
5. requires an explicit acknowledgement that v2 stopped;
6. restores the exact pre-switch v2-owned bytes;
...
There is no automatic release update. For a public exact-tag update:
...
v2-owned bytes plus exact Codex config/plugin bytes. A durable transaction
marker outside that snapshot makes recovery resumable after a manager-process
or operating-system interruption.
...
```bash
"$HOME/Library/Application Support/RobloxStudioMCPv2/bin/roblox-studio-mcp-v2-manage" rollback \
...
The Roblox tool catalog version is independent from the v2 release version.
Review a trusted local candidate first:

```bash
"$HOME/Library/Application Support/RobloxStudioMCPv2/bin/roblox-studio-mcp-v2-manage" catalog diff --artifact ./candidate-catalog.json
```
...
```bash
"$HOME/Library/Application Support/RobloxStudioMCPv2/bin/roblox-studio-mcp-v2-manage" catalog import \
...
Return every v2 Studio to Edit mode, then run:

```bash
"$HOME/Library/Application Support/RobloxStudioMCPv2/bin/roblox-studio-mcp-v2-manage" uninstall
```
