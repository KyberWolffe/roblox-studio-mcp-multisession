# Rc.5 to rc.7 isolated update and rollback proof

`scripts/prove_multisession_update_rollback.py` is the narrow real-package
release gate for `0.4.0-rc.7`. It is separate from, and does not change, the
historical rc.4-to-rc.5 rename/migration proof or rejected rc.6 provenance.
...
1. audits and extracts the real rc.5 and selected rc.7 archives;
2. verifies the claimed rc.7 Git commit exists, is a commit, resolves to the
...
5. runs the installed rc.7 doctor and proves rc.5 is the immediate rollback;
6. rolls back through the real rc.7 updater;
...
After the exact rc.7 candidate archive and source checkpoint exist, run:
...
python3 -B scripts/prove_multisession_update_rollback.py \
  ...
  --candidate-archive EXACT_RC7_ARCHIVE \
  --candidate-checksum-file EXACT_RC7_ARCHIVE.sha256 \
  --candidate-expected-sha256 EXACT_RC7_ARCHIVE_SHA256 \
  --candidate-version 0.4.0-rc.7 \
  --source-commit EXACT_RC7_COMMIT \
  --source-tree EXACT_RC7_TREE \
  --output EXTERNAL_ARTIFACT_DIR/MULTISESSION_RC5_RC7_ROLLBACK_PROOF.json
```
